...
<Dialog.Root bind:open onOpenChange={handleOpenChange}>
	<Dialog.Portal>
		<div transition:fade={{ duration: 200 }}>
			<Dialog.Overlay class="fixed inset-0 z-50 bg-md-onSurface/40 backdrop-blur-md" />
		</div>
		<div
			class="fixed left-[50%] top-[50%] z-50 w-full max-w-2xl translate-x-[-50%] translate-y-[-50%] focus:outline-none"
			transition:fly={{ y: 20, duration: 300 }}
		>
			<Dialog.Content class="bg-md-background border-2 border-md-onSurface p-0 shadow-2xl">
				<!-- Header -->
				<div class="p-8 pb-4 flex justify-between items-start border-b-2 border-md-onSurface/5">
					<div>
						...
						{#if subtitle}
							<Dialog.Description
								class="text-[10px] font-black uppercase tracking-[0.2em] text-md-onSurface/40 mt-1"
							>
								{subtitle}
							</Dialog.Description>
						{/if}
				...
				{#if footer}
					<div
						class="p-8 pt-4 border-t-2 border-md-onSurface/5 bg-md-surfaceVariant/10 flex justify-end gap-3 text-md-onSurface"
					>
						{@render footer()}
					</div>
				{/if}
