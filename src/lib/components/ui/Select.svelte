...
<div class="{className} space-y-2">
	...
	<Select.Root type="single" bind:value {items}>
		...
		<Select.Portal>
			<Select.Content
				class="z-[100] min-w-[var(--bits-select-anchor-width)] border-2 border-md-onSurface bg-md-background p-1 shadow-2xl animate-in fade-in zoom-in-95 duration-200"
				sideOffset={4}
			>
				<div class="max-h-60 overflow-y-auto custom-scrollbar">
					{#each items as item}
						<Select.Item
							value={item.value}
							label={item.label}
							class="flex items-center justify-between px-4 py-3 text-xs font-bold uppercase tracking-widest cursor-pointer outline-none hover:bg-md-onSurface hover:text-md-surface data-[selected]:bg-md-onSurface data-[selected]:text-md-surface transition-colors"
						>
							{item.label}
							{#if value === item.value}
