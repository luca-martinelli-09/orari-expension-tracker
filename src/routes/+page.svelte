...
<div class="flex flex-col gap-8">
	<div class="flex items-center justify-between">
		<div>
			<h2 class="text-3xl md:text-5xl font-black text-md-on-surface tracking-tighter uppercase">Calendario</h2>
			<p
	...
	<Card class="p-4 md:p-8 border-md-on-surface shadow-none rounded-none transition-colors duration-300">
			...
			{#each days as date}
				...
				{@const isNonWork = holiday || isWknd || dayData.type !== 'Lavoro'}
				                ...
				                <button
									...
									class="relative p-1 md:p-4 text-left transition-all duration-200 group flex flex-col justify-between aspect-square border-2 cursor-pointer
				                    {!isCurrentMonth
								...
								>
									<div class="flex justify-between items-start w-full">
										...
										{#if dayData.attachments.length > 0}
											<div class="border border-current rounded-full p-0.5 md:p-1 opacity-50">
												<Paperclip size={8} class="md:w-2.5 md:h-2.5" strokeWidth={3} />
											</div>
										{/if}
									</div>
									...
									<div class="flex flex-col gap-0.5 md:gap-1 mt-1 md:mt-2">
										{#if holiday}
											<div class="text-[7px] md:text-[9px] font-black uppercase tracking-tight truncate opacity-60">
												{holiday}
											</div>
										...
										{:else if hours > 0}
											<div class="text-[8px] md:text-[10px] font-black flex items-center gap-0.5 md:gap-1">
												<Clock size={8} class="md:w-2.5 md:h-2.5" strokeWidth={3} />
